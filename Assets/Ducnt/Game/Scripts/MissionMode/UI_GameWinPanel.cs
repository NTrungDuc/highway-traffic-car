...
public class UI_GameWinPanel : MonoBehaviour
{
    ...
    public UnlockBox unlockBox;
    private void OnEnable()
    {

        HR_GamePlayHandler.OnPlayerWon += HR_PlayerHandler_OnPlayerWon;
    ...
    private void HR_PlayerHandler_OnPlayerWon(HR_PlayerHandler player, int[] scores)
    {
    ...
    public IEnumerator showUnlockBox(HR_PlayerHandler player, int[] scores)
    {
        if(HR_GamePlayHandler.Instance.mode == HR_GamePlayHandler.Mode.MissionMode_1 || HR_GamePlayHandler.Instance.mode == HR_GamePlayHandler.Mode.MissionMode_2)
        {
            ...
            if (PlayerPrefs.GetFloat("ProgressBox") != 0.8f)
            {
                StartCoroutine(DisplayResults(player, scores, 3f));
            }
            else
            {
                unlockBox.BtnClaim.GetComponent<Button>().onClick.AddListener(() => { StartCoroutine(DisplayResults(player, scores, 0f)); });
                unlockBox.BtnNoThanks.GetComponent<Button>().onClick.AddListener(() => { StartCoroutine(DisplayResults(player, scores, 0f)); });
            }
        }
        else if(HR_GamePlayHandler.Instance.mode == HR_GamePlayHandler.Mode.AIRacing)
            StartCoroutine(DisplayResults(player, scores, 1f));

    }
    public IEnumerator DisplayResults(HR_PlayerHandler player, int[] scores, float timeShow)
    {
        yield return new WaitForSecondsRealtime(timeShow);

        content.SetActive(true);

        float currentRankPoint = PlayerPrefs.GetFloat("CurrentRankPoint") + (scores[4] * 0.1f);
        ...
        if(HR_GamePlayHandler.Instance.mode == HR_GamePlayHandler.Mode.AIRacing)
        {
            ...
            if (bonusText)
                bonusText.text = (PlayerPrefs.GetInt("priceModeDual") * 2).ToString();
        ...
        else
        {
            ...
            totalDistance.text = (player.distanceWinMM * 1000).ToString("F2");
