...
public class RCC_CarControllerV3 : RCC_Core
{
    ...
    public Transform FrontLeftRimTransform;
    public Transform FrontLeftTireTransform;
    public Transform FrontLeftWheelTransform;
    public Transform FrontRightRimTransform;
    public Transform FrontRightTireTransform;
    public Transform FrontRightWheelTransform;
    public Transform RearLeftRimTransform;
    public Transform RearLeftTireTransform;
    public Transform RearLeftWheelTransform;
    public Transform RearRightRimTransform;
    ...
    public float speed = 35f;                                                    // Vehicle speed in km/h or mp/h.
    ...
    [HideInInspector] public float throttleInputAI = 0f;
    [HideInInspector] public float brakeInput = 0f;
    [HideInInspector] public float steerInput = 0f;
    [HideInInspector] public float steerInputAI = 0f;
    ...
    public bool hasUseNitro = false;
    public float NitroTime = 3;
    public float nitroBoost = 50;
    ...
    public HR_PlayerHandler playerHandler;
    public HR_AICarNew CarAIHandler;
    public bool isCheckNitroAI;
    float timeRunNitroAI;
    private void Awake()
    {
        ...
        AllWheelColliders[0].WheelCollider.ConfigureVehicleSubsteps(configureVehicleSubsteps.speedThreshold, configureVehicleSubsteps.stepsBelowThreshold, configureVehicleSubsteps.stepsAboveThreshold);

        // Assigning wheel models of the wheelcolliders.
        FrontLeftWheelCollider.rimModel = FrontLeftRimTransform;
        FrontLeftWheelCollider.tireModel = FrontLeftTireTransform;
        FrontLeftWheelCollider.wheelModel = FrontLeftWheelTransform;
        FrontRightWheelCollider.rimModel = FrontRightRimTransform;
        FrontRightWheelCollider.tireModel = FrontRightTireTransform;
        FrontRightWheelCollider.wheelModel = FrontRightWheelTransform;
        RearLeftWheelCollider.rimModel = RearLeftRimTransform;
        RearLeftWheelCollider.tireModel = RearLeftTireTransform;
        RearLeftWheelCollider.wheelModel = RearLeftWheelTransform;
        RearRightWheelCollider.rimModel = RearRightRimTransform;
        RearRightWheelCollider.tireModel = RearRightTireTransform;
    ...
    private void Start()
    {
        if (!isCheckAi)
            ...
            CarAIHandler = GetComponent<HR_AICarNew>();
    }
    ...
    private void OnEnable()
    {
        ...
        RCC_InputManager.OnNitro += RCC_InputManager_OnNitro;
    ...
    private void CreateAudios()
    {
        ...
        nitroSound = NewAudioSource(RCC_Settings.Instance.audioMixer, gameObject, "Nitro AudioSource", 5, 30, .75f, SoundCarManager.Instance.nitroSound, false, false, false);
    }
        ...
        if (SteeringWheel)
        {
            ...
            else
            {
                switch (steeringWheelRotateAround)
                {
                    ...
                    case SteeringWheelRotateAround.XAxis:
                        SteeringWheel.transform.localRotation = orgSteeringWheelRot * Quaternion.AngleAxis(((steerInputAI * steerAngle) * -steeringWheelAngleMultiplier), Vector3.right);
                        break;

                    case SteeringWheelRotateAround.YAxis:
                        SteeringWheel.transform.localRotation = orgSteeringWheelRot * Quaternion.AngleAxis(((steerInputAI * steerAngle) * -steeringWheelAngleMultiplier), Vector3.up);
                        break;

                    case SteeringWheelRotateAround.ZAxis:
                        SteeringWheel.transform.localRotation = orgSteeringWheelRot * Quaternion.AngleAxis(((steerInputAI * steerAngle) * -steeringWheelAngleMultiplier), Vector3.forward);
                        break;
    ...
    private void Update()
    {
        ...
        if (isCheckAi)
        {
            //Debug.Log("viet_isCheckNitro: " + HR_GamePlayHandler.Instance.isCheckNitro);
            
            // khi đi sau xe player mà player dùng nitro thì mới cho xe AI dùng nitro
            if (!HR_GamePlayHandler.Instance.carAIGameHR_AICar.isCheckStart)
                return;
            if (HR_GamePlayHandler.Instance != null && HR_GamePlayHandler.Instance.isCheckNitro && HR_GamePlayHandler.Instance.isCheckAIAfter && !HR_GamePlayHandler.Instance.isCheckAIBefore && HR_GamePlayHandler.Instance.distancePlayerBetweenAI > 30)
            {
                ApplyNitro();
            }
            //else if (HR_GamePlayHandler.Instance != null && HR_GamePlayHandler.Instance.isCheckAIAfter && !HR_GamePlayHandler.Instance.isCheckAIBefore && HR_GamePlayHandler.Instance.distancePlayerBetweenAI < 30 && speed > 100)
            //{
            //    brakeInput = 1;
            //    throttleInputAI = 0;
            //    //Debug.Log("viet_tren 100");
            //}
            //else if (HR_GamePlayHandler.Instance != null && HR_GamePlayHandler.Instance.isCheckAIAfter && !HR_GamePlayHandler.Instance.isCheckAIBefore && HR_GamePlayHandler.Instance.distancePlayerBetweenAI > 30)
            //{
            //    brakeInput = 0;
            //    throttleInputAI = 1;
            //    //Debug.Log("viet_duoi 100");

            //}
            SteeringAssistance();

            float angle = HR_CalculateAngle.CalculateAngle(transform.rotation, Quaternion.identity * Quaternion.AngleAxis(steerInputAI * steerAngle, Vector3.up));
            steerInputAI = (angle / 25f) * direction;       // tốc độ chuyển sang trái sang phải

            if (HR_GamePlayHandler.Instance && speed < 70f && isCheckAi )
            {
                throttleInputAI = 1;
                //brakeInput = 0f;
                handbrakeInput = 0f;
        ...
        //FOV Gas
        if (HR_GamePlayHandler.Instance != null && !isCheckAi)
        {
            if (speed >= 80)
            {
                HR_CarCamera.Instance.FOVGas(57);
                HR_CarCamera.Instance.activeWindEffect(true,playerHandler);
                playerHandler.ShowEffectNitro(true);
                playerHandler.ShowEffectTrailLight(true);
            }
            else if (speed >= 50 && speed < 80)
            {
                HR_CarCamera.Instance.FOVGas(47);
                HR_CarCamera.Instance.activeWindEffect(false,playerHandler);
                playerHandler.ShowEffectNitro(false);
                playerHandler.ShowEffectTrailLight(false);
            }
        }
        //player
        Inputs();
        ...
        else if (throttleInput < .1f && transform.InverseTransformDirection(Rigid.velocity).z > -1f && direction == -1 && !changingGear && AutomaticGear && !semiAutomaticGear && !RCC_InputManager.Instance.logitechHShifterUsed && !isCheckAi)
            StartCoroutine(ChangeGear(0));
        else if (throttleInputAI < .1f && transform.InverseTransformDirection(Rigid.velocity).z > -1f && direction == -1 && !changingGear && AutomaticGear && !semiAutomaticGear && !RCC_InputManager.Instance.logitechHShifterUsed && isCheckAi)
            StartCoroutine(ChangeGear(0));
        ...
        {
            if (throttleInputAI >= .1f)
                launched += throttleInputAI * Time.deltaTime;
            else
                launched -= Time.deltaTime;
        }
    ...
    private void FixedUpdate()
    {
        ...
        if (RCC_Settings.Instance.selectedBehaviorType != null && RCC_Settings.Instance.selectedBehaviorType.applyRelativeTorque)
        {
            ...
            else if (isGrounded && isCheckAi)
                Rigid.AddRelativeTorque(Vector3.up * (((steerInputAI * throttleInputAI) * direction)) * Mathf.Lerp(1.5f, .5f, speed / 200f), ForceMode.Acceleration);
        }
        ...
        //    if (speed >= 80)
        //    {
        //        HR_CarCamera.Instance.FOVGas(80);
        //        HR_CarCamera.Instance.windEffect.Play();
        //    }
        //    else if (speed >= 50 && speed < 80)
        //    {
        //        HR_CarCamera.Instance.FOVGas(60);
        //        HR_CarCamera.Instance.windEffect.Stop();
        //    }
        //}
    }
    ...
    private void Inputs()
    {
        if (!isCheckAi)
        {
            if (canControl)
            {
                ...
                if (!externalController)
                {
                    ...
                    steerInput = (angle / 15f) * direction;       // tốc độ chuyển sang trái sang phải
        ...
        if (!useNOS || NoS < 5 || throttleInput < .75f)
            ...
            if (HR_GamePlayHandler.Instance && speed < 70f && !isCheckAi && HR_GamePlayHandler.Instance.gameStarted)
            ...
            if (useNitro)
                steerInput = Mathf.Clamp(steerInput, -0.4f, 0.4f);
            else
    ...
    private void SteeringAssistance()
    {
        ...
        if (useSteeringLimiter)
        {
            ...
            else
            {
                //	If slip is high enough, apply counter input.
                if (maxSteerInput > 0f)
                    steerInputAI = Mathf.Clamp(steerInputAI, -maxSteerInput, maxSteerInput);
                else
                    steerInputAI = Mathf.Clamp(steerInputAI, sign * maxSteerInput, sign * maxSteerInput);
            }
    ...
    private void Engine()
    {
        ...
        {
            engineRPMRaw = Mathf.SmoothDamp(engineRPMRaw, (Mathf.Lerp(minEngineRPM, maxEngineRPM + 500f, (clutchInput * throttleInputAI)) +
            ((wheelRPM / Mathf.Clamp(poweredWheels, 1, Mathf.Infinity)) *
            finalRatio * (gears[currentGear].maxRatio)) * (1f - clutchInput)) *
            fuelInput, ref velocity, newEngineInertia * .75f);
        }
    ...
    public void EngineSounds()
    {
        ...
        if (!isCheckAi)
            ...
            volumeLevel = Mathf.Clamp(throttleInputAI, 0f, 1f);
    ...
    private void Wheels()
    {
        ...
        for (int i = 0; i < AllWheelColliders.Length; i++)
        {
            ...
            {
                if (AllWheelColliders[i].canPower)
                    AllWheelColliders[i].ApplyMotorTorque((direction * AllWheelColliders[i].powerMultiplier * (1f - clutchInput) * throttleInputAI * (1f + boostInput) * (engineTorqueCurve.Evaluate(engineRPM) * gears[currentGear].maxRatio * finalRatio)) / Mathf.Clamp(poweredWheels, 1, Mathf.Infinity));
                if (AllWheelColliders[i].canSteer)
                    AllWheelColliders[i].ApplySteering(steerInputAI * AllWheelColliders[i].steeringMultiplier, steerAngle);
            }
        ...
        if (currentGear == 0)
        {
            ...
            if (launched >= .25f)
            {
                if (!isCheckAi)
                    ...
                    clutchInput = Mathf.Lerp(clutchInput, (Mathf.Lerp(1f, (Mathf.Lerp(clutchInertia, 0f, (wheelRPM / Mathf.Clamp(poweredWheels, 1, Mathf.Infinity)) / gears[0].targetSpeedForNextGear)), Mathf.Abs(throttleInputAI))), Time.fixedDeltaTime * 20f);
    ...
    private void GearBox()
    {
        ...
        if (AutomaticGear && !RCC_InputManager.Instance.logitechHShifterUsed)
        {
    ...
    private void Nitro()
    {
        ...
        else
        {
            nitroSound.Stop();
            hasUseNitro = true;
            HR_CarCamera.Instance.speedEffects.gameObject.SetActive(false);
            if (NitroTime + (NitroTime * 0.5f) < utimer)
            {
                utimer = 0f;
                useNitro = false;
                Debug.Log("end nitro");
                ...
                Rigid.velocity -= new Vector3(0f, 0f, Rigid.velocity.z * 0.0015f);
        }
    }
    ...
    void ApplyNitro()
    {
        ...
        if (speed < maxspeed + (maxspeed * .2f))
        {
            Rigid.velocity += new Vector3(0f, 0f, Rigid.velocity.z * 0.025f);
            //Rigid.AddForce(transform.forward * nitroBoost, ForceMode.Acceleration);
            //transform.position = new Vector3(transform.position.x, Mathf.Clamp(yPosBeforeNitro, yPosBeforeNitro - 0.125f, yPosBeforeNitro + 0.125f), transform.position.z);
        }
        //else
        //{
        //    float remainingSpeed = maxspeed - speed;
        //    float adjustedNitro = remainingSpeed / 3.6f;
        //    Rigid.AddForce(transform.forward * Mathf.Min(adjustedNitro, nitroBoost), ForceMode.Acceleration);
        //}
    }
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        for (int i = 0; i < collision.contacts.Length; i++)
            force += collision.contacts[i].impulse;
        ...
        if (((1 << collision.gameObject.layer) & damage.damageFilter) != 0)
        {
            ...
            //delay time scale
            HPS_TrafficCar trafficCar = collision.gameObject.GetComponent<HPS_TrafficCar>();
            if (!isRammed && !isCheckAi)
            {
            ...
            if (trafficCar != null && !isCheckAi)
            {
                //Debug.Log(playerHandler.currentTrafficCarNameBack);
                //huc xe va gameover
                if (playerHandler.currentTrafficCarNameBack != null && !useNitro & playerHandler.currentTrafficCarNameLeft == null
                    && playerHandler.currentTrafficCarNameRight == null ||
                    playerHandler.currentTrafficCarNameLeft == null
                    && playerHandler.currentTrafficCarNameRight == null
                    && playerHandler.currentTrafficCarNameBack == null && !useNitro)
                {
                    if (playerHandler.gameWin)
                    {
                        return;
                    }
                    //Debug.Log("GameOver");
                    ...
                    {
                        playerHandler.GameFail();
                    }
                    else if (HR_GamePlayHandler.Instance.mode == HR_GamePlayHandler.Mode.AIRacing)
                    {
                        playerHandler.GamePause(true);
                    }
                    else
                    {
                        playerHandler.GameOver();
                    }
                }

                if (!playerHandler.crashed)
                {
                    if (speed > 70)
                    {
                        trafficCar.Rammed(playerHandler.currentTrafficCarNameLeft != null, playerHandler.currentTrafficCarNameRight != null, playerHandler.opposideDirectionCurrent > .1f);
            ...
            else if (trafficCar != null && isCheckAi)
            {
                if (CarAIHandler.currentTrafficCarNameLeft != null && CarAIHandler.currentTrafficCarNameRight == null || CarAIHandler.currentTrafficCarNameLeft == null && CarAIHandler.currentTrafficCarNameRight != null)
                {
                    if (speed > 70)
                        trafficCar.RammedAi(CarAIHandler.currentTrafficCarNameLeft != null, CarAIHandler.currentTrafficCarNameRight != null);
                }
            ...
            if (useCollisionAudio)
            {
                ...
                if (CrashClips.Length > 0)
                {
                    ...
                    if (collision.GetContact(0).thisCollider.gameObject.transform != transform.parent)
                    {
                        ...
                        crashSound = NewAudioSource(RCC_Settings.Instance.audioMixer, gameObject, "Crash Sound AudioSource", 0f, 0f, RCC_Settings.Instance.maxCrashSoundVolume * (collision.impulse.magnitude / 10000f), CrashClips[UnityEngine.Random.Range(0, CrashClips.Length)], false, true, true);
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("Item"))
        {
            if (playerHandler != null)
            {
                ...
                {
                    coinSound = NewAudioSource(RCC_Settings.Instance.audioMixer, gameObject, "Coin AudioSource", 5, 25, .35f, SoundCarManager.Instance.coinSound, false, false, false);
                    coinSound.Play();
    ...
    public IEnumerator NitroBehavior()
    {
        useNitro = true;
        cooldownTimer = 0;
        hasUseNitro = false;
        HR_CarCamera.Instance.CameraBack(playerHandler);
        nitroSound.Play();
        ...
        //playerHandler.ShowEffectNitro(true);
        HR_CarCamera.Instance.speedEffects.gameObject.SetActive(true);

        yield return null;
    }

    private void RCC_InputManager_OnNitro()
    {
        if (!canNitro)
            return;

        StartCoroutine(NitroBehavior());
    }

    public void RCC_InputManager_OnNitro(float cooldownTime)
    {
        if (!canNitro)
            return;
    ...
    private void Reset()
    {
        ...
        HR_PlayerHandler playerHandler = GetComponent<HR_PlayerHandler>();

        if (!playerHandler)
            playerHandler = gameObject.AddComponent<HR_PlayerHandler>();
