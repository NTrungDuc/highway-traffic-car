...
public class RCC_Customization : MonoBehaviour {
    ...
    /// <summary>
    /// Change Rim Models. You can find your rim models array in Tools --> BCG --> RCC --> Configure Changable Rims.
    /// </summary>
    public static void ChangeRims(RCC_CarControllerV3 vehicle, GameObject rim, bool applyRadius)
    {
        ...
        for (int i = 0; i < vehicle.AllWheelColliders.Length; i++)
        {
            ...
            //  Disabling renderer of the rimmodel.
            if (vehicle.AllWheelColliders[i].rimModel.GetComponentInChildren<MeshRenderer>())
                vehicle.AllWheelColliders[i].rimModel.GetComponentInChildren<MeshRenderer>().enabled = false;

            //  Disabling all rim models of the wheel.
            foreach (Transform t in vehicle.AllWheelColliders[i].rimModel.parent.GetComponentInChildren<Transform>())
            {
                if (t.name.Contains("Rim"))
                    t.gameObject.SetActive(false);
            }

            //  Instantiating new wheel.
            GameObject newRim = Instantiate(rim, vehicle.AllWheelColliders[i].rimModel.position, vehicle.AllWheelColliders[i].rimModel.rotation, vehicle.AllWheelColliders[i].rimModel.parent);

            //  If wheel is at right side, multiply scale X by -1 for symetry.
            //if (vehicle.AllWheelColliders[i].rimModel.localPosition.x > 0f)
            //    newRim.transform.localRotation *= Quaternion.Euler(0f, 180f, 0f);
        }
    ...
    /// <summary>
    /// Change Tire Models. You can find your tire models array in Tools --> BCG --> RCC --> Configure Changable Tires.
    /// </summary>
    public static void ChangeTires(RCC_CarControllerV3 vehicle, GameObject tire, bool applyRadius)
    {
        ...
        for (int i = 0; i < vehicle.AllWheelColliders.Length; i++)
        {
            ...
            //  Disabling renderer of the tiremodel.
            if (vehicle.AllWheelColliders[i].tireModel.GetComponentInChildren<MeshRenderer>())
                vehicle.AllWheelColliders[i].tireModel.GetComponentInChildren<MeshRenderer>().enabled = false;

            //  Disabling all tire models of the wheel.
            foreach (Transform t in vehicle.AllWheelColliders[i].tireModel.parent.GetComponentInChildren<Transform>())
            {
                if (t.name.Contains("Tire"))
                    t.gameObject.SetActive(false);
            }

            //  Instantiating new tire.
            GameObject newTire = Instantiate(tire, vehicle.AllWheelColliders[i].tireModel.position, vehicle.AllWheelColliders[i].tireModel.rotation, vehicle.AllWheelColliders[i].tireModel.parent);

            //  If tire is at right side, multiply scale X by -1 for symetry.
            if (vehicle.AllWheelColliders[i].tireModel.localPosition.x > 0f)
                newTire.transform.localRotation *= Quaternion.Euler(0f, 180f, 0f);

            //  If apply radius is set to true, calculate the radius.
            if (applyRadius)
                vehicle.AllWheelColliders[i].WheelCollider.radius = RCC_GetBounds.MaxBoundsExtent(tire.transform);
